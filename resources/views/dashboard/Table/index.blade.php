...
<html lang="en">
   ...
   <body class="dashboard dashboard_1">
     ...
     <!-- table -->
     <br>
     <a href="{{action('App\Http\Controllers\Admin\TableController@create')}}" class="btn btn-danger">Add Table</a>
<br>
     <table class="table table-striped">
  <thead>
    <tr>
      <th scope="col">Name</th>
      <th scope="col">Guset_number</th>
  ...
  @foreach($table as $tables)
    <tr>
      <td>{{$tables->name}}</td>
      <td>{{$tables->guest_number}}</td>
      <td>{{$tables->price}}</td>
      <td>{{$tables->status}}</td>
      <td>{{$tables->location}}</td>

      <td>
    <form action="{{ action('App\Http\Controllers\Admin\TableController@destroy', $tables->id) }}" method="POST" onsubmit="return confirm('Are you sure')">
        @csrf
        @method('DELETE')
        <a href="{{ action('App\Http\Controllers\Admin\TableController@edit', $tables->id) }}" class="btn btn-warning" style="background: transparent; border: none;">
